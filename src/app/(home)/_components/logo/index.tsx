...
export const Logo: React.FC<React.SVGProps<SVGSVGElement>> = () => {
  ...
  const [idleAnimationActive, setIdleAnimationActive] = useState(false)
  ...
  useEffect(() => {
    ...
    return () => {
      newParticles.forEach((p) => {
        try {
          if (containerRef.current && containerRef.current.contains(p)) {
            containerRef.current.removeChild(p)
          }
        } catch (error) {
          console.debug('Particle cleanup error handled gracefully')
        }
      })
    }
  }, [])
  ...
  useEffect(() => {
    ...
    const cleanupQueue: { element: HTMLDivElement; timeout: NodeJS.Timeout }[] =
      []

    // Helper function to remove elements from DOM after they're not visible
    const scheduleCleanup = (element: HTMLDivElement) => {
      // Schedule removal after the animation should be completed (typically 5-7 seconds)
      const timeout = setTimeout(() => {
        try {
          if (containerRef.current && containerRef.current.contains(element)) {
            containerRef.current.removeChild(element)
            // Remove from leaves array if it exists there
            const leafIndex = newLeaves.indexOf(element)
            if (leafIndex > -1) {
              newLeaves.splice(leafIndex, 1)
            }
          }
          // Remove from cleanup queue
          const queueIndex = cleanupQueue.findIndex(
            (item) => item.element === element
          )
          if (queueIndex > -1) {
            cleanupQueue.splice(queueIndex, 1)
          }
        } catch (error) {
          console.debug('Leaf cleanup error handled gracefully')
      ...
      }, 8000) // 8 seconds should be enough for most animations to complete

      cleanupQueue.push({ element, timeout })
    }
    ...
    const leafInterval = setInterval(() => {
      if (newLeaves.length >= 15) {
        ...
        if (oldestLeaf && containerRef.current) {
          try {
            if (containerRef.current.contains(oldestLeaf)) {
              containerRef.current.removeChild(oldestLeaf)
            }
          } catch (error) {
            console.debug('Oldest leaf cleanup error handled gracefully')
          }
        }
      }
    ...
    return () => {
      ...
      cleanupQueue.forEach((item) => clearTimeout(item.timeout))
      // Remove all leaves
      newLeaves.forEach((leaf) => {
        try {
          ...
          console.debug('Leaf return cleanup error handled gracefully')
        }
      })
    }
  }, [isAnimationComplete])

  // Thêm hiệu ứng idle animation - tạo thêm lá và nước khi để yên
  useEffect(() => {
    if (!isAnimationComplete || !containerRef.current) return

    // Lưu thời điểm cuối cùng tương tác
    const initialTime = Date.now()
    setLastInteractionTime(initialTime)

    // Reference to track idle state within this effect to avoid dependency cycles
    const idleStateRef = { active: false }

    // Function để kiểm tra trạng thái idle
    const checkIdleState = () => {
      const now = Date.now()
      const idleTime = now - lastInteractionTime

      // Nếu để yên 5 giây, bắt đầu hiệu ứng idle
      if (idleTime > 5000 && !idleStateRef.active) {
        idleStateRef.active = true
        setIdleAnimationActive(true)
      }
    }

    // Kiểm tra idle state mỗi 2 giây
    const idleCheckInterval = setInterval(checkIdleState, 2000)

    // Tạo hiệu ứng khi idle
    const idleAnimationInterval = setInterval(() => {
      // Use the ref to check idle state instead of the state variable
      if (!idleAnimationActive || !containerRef.current) return

      // Tạo lá hoặc giọt nước ngẫu nhiên
      const createCount = Math.floor(Math.random() * 3) + 1 // 1-3 phần tử mỗi lần

      for (let i = 0; i < createCount; i++) {
        setTimeout(() => {
          if (containerRef.current && idleAnimationActive) {
            const leaf = createLeaf(
              containerRef.current,
              leaves,
              createWaterSplash
            ...
            if (leaf) {
              ...
              setTimeout(() => {
                try {
                  if (
                    containerRef.current &&
                    containerRef.current.contains(leaf)
                  ) {
                    containerRef.current.removeChild(leaf)
                  }
                } catch (error) {
                  console.debug(
                    'Idle animation leaf cleanup error handled gracefully'
                  )
                }
              }, 8000)
            }
          }
        }, i * 300) // Tạo các phần tử cách nhau 300ms
      }
    }, 2000) // Tạo hiệu ứng mỗi 2 giây khi idle

    // Xử lý sự kiện tương tác để reset idle state
    const handleInteraction = () => {
      setLastInteractionTime(Date.now())
      if (idleStateRef.active) {
        idleStateRef.active = false
        setIdleAnimationActive(false)
      }
    }

    // Thêm các event listener để theo dõi tương tác
  ...
  }, [isAnimationComplete, leaves]) // Remove the dependency on idleAnimationActive and lastInteractionTime
  ...
  const handleClick = () => {
    ...
    // Reset idle timer khi tương tác
    setLastInteractionTime(Date.now())
    setIdleAnimationActive(false)
          ...
          if (leaf) {
            ...
            setTimeout(() => {
              try {
                ...
                console.debug('Click leaf cleanup error handled gracefully')
              }
            }, 8000)
          }
        }
  ...
  const handleMouseMove = () => {
    ...
    // Reset idle state khi có tương tác
    setLastInteractionTime(Date.now())
    setIdleAnimationActive(false)

    // Thỉnh thoảng tạo một giọt nước ngẫu nhiên khi di chuột
    if (Math.random() < 0.03) {
      // 3% cơ hội mỗi lần di chuột
      const leaf = createLeaf(containerRef.current, leaves, createWaterSplash)
      if (leaf) {
        // Set class riêng cho loại này
        leaf.className = 'water-drop-element'

        // Đảm bảo là giọt nước
        leaf.style.width = `${Math.random() * 12 + 8}px`
        leaf.style.height = `${Math.random() * 12 + 8}px`
        ...
        setTimeout(() => {
          try {
            ...
            console.debug('Mouse move leaf cleanup error handled gracefully')
          }
        }, 8000)
      }
    }
